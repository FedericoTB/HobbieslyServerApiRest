...
class UserManager(BaseUserManager):
    def _create_user(self, username, email, password, is_active, is_staff, is_superuser, **extrafields):
        ...
        user = self.model(username=username, email=email, is_active=True, is_staff=is_staff, is_superuser=is_superuser,
                          date_joined=timezone.now(), **extrafields)
...
class User(AbstractBaseUser, PermissionsMixin):
    ...
    adress = models.CharField(max_length=300,default="default adress")
...
class UsersGroups(models.Model):
    ...
    image_url = models.URLField(default="https://cdn0.iconfinder.com/data/icons/"
                                "users-android-l-lollipop-icon-pack/24/group2-512.png")
...
class Classification(models.Model):
    objects = models.Manager
    category = models.ForeignKey(Categories, related_name="clas_category", on_delete=models.SET_NULL, blank=True,
                                 null=True)
